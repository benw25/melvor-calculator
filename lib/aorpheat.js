...
export const calcAorpheatSingleAction = (level) => {
  return calcAorpheat(level, 1);
};

export const calcAorpheatHour = (level, actionTime) => {
  const iterations = _.divide(HOUR_IN_SECONDS, actionTime);

  return calcAorpheat(level, iterations);
};

export const calcAorpheatDay = (level, actionTime) => {
  const iterations = _.divide(DAY_IN_SECONDS, actionTime);

  return calcAorpheat(level, iterations);
};

export const calcAorpheatWeek = (level, actionTime) => {
...
BigDecimal.decimals = PRECISION_FACTOR; // Configuration of the number of decimals you want to have.
